...
void TileWidget::fromJSON(nlohmann::json j)
{
    ...
    if(split) {
        ...
        // real sizes are only known once shown, resizeEvent() takes care of fitTiles()
...
void TileWidget::resizeEvent(QResizeEvent *event)
{
    ...
    // the splitter gets its new geometry through the layouts, adjust it afterwards
    QMetaObject::invokeMethod(this, [this]() { fitTiles(); }, Qt::QueuedConnection);
}

void TileWidget::fitTiles()
{
    if(!isSplit || !splitter || splitter->orientation() != Qt::Horizontal) {
        return;
    }
    int total = splitter->width();
    int h = splitter->height();
    // the neighbouring plot must keep a usable width; if the row is too narrow
    // for both, leave the proportional arrangement alone
    constexpr int minNeighbourWidth = 200;
    if(total < 2 * minNeighbourWidth || h <= 0) {
        return;
    }
    auto wanted = [&](TileWidget *c) -> int {
        if(!c || !c->hasContent || !c->content) {
            return -1;
        }
        int w = c->content->preferredTileWidth(h);
        if(w < 0) {
            return -1;
        }
        return qBound(minNeighbourWidth / 2, w, total - minNeighbourWidth);
    };
    int w1 = wanted(child1);
    int w2 = wanted(child2);
    QList<int> sizes;
    if(w1 >= 0 && w2 < 0) {
        sizes = {w1, total - w1};
    } else if(w2 >= 0 && w1 < 0) {
        ...
        // neither (or both) children ask for a fixed width, leave the user's arrangement
        return;
    }
    if(splitter->sizes() != sizes) {
...
void TileWidget::addPlotInNewTile(TracePlot *plot)
{
    if(isSplit) {
        child2->addPlotInNewTile(plot);
        return;
    }
    if(!hasContent) {
        setContent(plot);
...
void TileWidget::setContent(TracePlot *plot)
{
    ...
    connect(content, &TracePlot::tileFitChanged, this, [=]() {
        if(parent) {
            parent->fitTiles();
        }
    }, Qt::QueuedConnection);
...
void TileWidget::removeContent()
{
    if(hasContent) {
        ...
        disconnect(content, &TracePlot::tileFitChanged, this, nullptr);
        hasContent = false;
        content = nullptr;
