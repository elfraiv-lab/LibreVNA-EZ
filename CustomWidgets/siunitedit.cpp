...
bool SIUnitEdit::eventFilter(QObject *, QEvent *event)
{
    if (event->type() == QEvent::KeyPress) {
        ...
        if(Preferences::getInstance().UI.tabletMode && isEnabled() && !isReadOnly()) {
            // touch input: open on-screen keypad instead of relying on the OS keyboard
            double v;
            if(NumPadDialog::getValue(this, unit, prefixes, _value, precision, v, maxDecimals)) {
                clear();
                setValue(v);
            } else {
                clear();
...
{
    _value = value;
    setPlaceholderText(Unit::ToString(value, unit, prefixes, precision, maxDecimals));
    if(!text().isEmpty()) {
...
{
    // edit the full-precision value, not the (possibly rounded) displayed text
    setText(Unit::ToString(_value, unit, prefixes, precision));
    selectAll();
}
