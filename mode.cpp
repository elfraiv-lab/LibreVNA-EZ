...
void Mode::activate()
{
    ...
    // restore dock and toolbar positions (separate arrangement per layout mode)
    const bool tablet = Preferences::getInstance().UI.tabletMode;
    const QString layoutKey = tablet ? "_tablet" : "";
    window->PrepareLayoutRestore();
    bool restored = window->restoreState(settings.value("windowState_"+name+layoutKey).toByteArray());

    // restore visibility of toolbars and docks
    for(auto d : docks) {
        bool hidden = settings.value("dock_"+name+layoutKey+"_"+d->windowTitle(), d->isHidden()).toBool();
        if(hidden) {
            d->hide();
        } else {
            d->show();
    ...
    for(auto t : toolbars) {
        bool hidden = settings.value("toolbar_"+name+layoutKey+"_"+t->windowTitle(), t->isHidden()).toBool();
        if(hidden) {
            t->hide();
        } else {
            t->show();
    ...
    if(!restored) {
        // no saved arrangement for this layout mode yet, use defaults
        if(tablet) {
            window->ApplyTabletLayout();
        } else {
            window->ApplyDesktopLayout();
        }
    }
    window->AfterLayoutRestore();
...
{
    QSettings settings;
    const QString layoutKey = Preferences::getInstance().UI.tabletMode ? "_tablet" : "";
    // save dock/toolbar visibility
    for(auto d : docks) {
        settings.setValue("dock_"+name+layoutKey+"_"+d->windowTitle(), d->isHidden());
    }
    for(auto t : toolbars) {
        settings.setValue("toolbar_"+name+layoutKey+"_"+t->windowTitle(), t->isHidden());
    }
    settings.setValue("windowState_"+name+layoutKey, window->saveState());
