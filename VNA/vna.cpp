...
#include "Deembedding/portextension.h"
#include "Traces/Math/tdr.h"
#include "Traces/Math/expression.h"
#include "ui_main.h"
...
//    portExtension.setCalkit(&cal.getCalibrationKit());
    ...
    auto antennaBand = toolsMenu->addAction("Antenna band...");
    antennaBand->setToolTip("Set sweep, VSWR limit line and antenna band marker for a required frequency band");
    connect(antennaBand, &QAction::triggered, this, &VNA::StartAntennaBandSetup);
    auto antennaReport = toolsMenu->addAction("Antenna report (PDF)...");
    antennaReport->setToolTip("Save a PDF report with setup, VSWR summary, markers and graphs");
    connect(antennaReport, &QAction::triggered, this, &VNA::StartAntennaReport);
    auto cableImpedance = toolsMenu->addAction("Cable impedance (TDR)...");
    cableImpedance->setToolTip("Measure the characteristic impedance of a cable section from the S11 step response");
    connect(cableImpedance, &QAction::triggered, this, &VNA::StartCableImpedanceSetup);
    auto cableLoss = toolsMenu->addAction("Cable loss (open/short)...");
    cableLoss->setToolTip("Measure the loss of a cable with its far end open or shorted (one port)");
    connect(cableLoss, &QAction::triggered, this, &VNA::StartCableLossSetup);
    ...
    // Edit toolbar: undo/redo of the setup (sweep, traces, graphs, markers), see UndoHistory
    auto tb_edit = new QToolBar("Edit");
    auto aUndo = new QAction(QIcon(":/icons/undo.svg"), "Undo", this);
    aUndo->setShortcut(QKeySequence::Undo);
    aUndo->setToolTip("Undo the last change of sweep, traces, graphs or markers (Ctrl+Z)");
    aUndo->setEnabled(false);
    auto aRedo = new QAction(QIcon(":/icons/redo.svg"), "Redo", this);
    aRedo->setShortcuts({QKeySequence::Redo, QKeySequence("Ctrl+Y"), QKeySequence("Ctrl+Shift+Z")});
    aRedo->setToolTip("Redo (Ctrl+Y)");
    aRedo->setEnabled(false);
    tb_edit->addAction(aUndo);
    tb_edit->addAction(aRedo);
    window->addToolBar(tb_edit);
    toolbars.insert(tb_edit);
    // shortcuts must work even when the toolbar is hidden
    window->addAction(aUndo);
    window->addAction(aRedo);
    actions.insert(aUndo);
    actions.insert(aRedo);
    undoHistory = new UndoHistory([this]() { return toJSON(); },
                                  [this](const nlohmann::json &j) { fromJSON(j); }, this);
    connect(undoHistory, &UndoHistory::availabilityChanged, this, [=](bool canUndo, bool canRedo) {
        aUndo->setEnabled(canUndo);
        aRedo->setEnabled(canRedo);
    });
    connect(aUndo, &QAction::triggered, undoHistory, &UndoHistory::undo);
    connect(aRedo, &QAction::triggered, undoHistory, &UndoHistory::redo);
    ...
    compactHiddenActions.push_back(tb_sweep->addWidget(new QLabel("Sweep type:")));
    cbSweepType = new QComboBox();
    cbSweepType->addItem("Frequency");
    cbSweepType->addItem("Power");
    compactHiddenActions.push_back(tb_sweep->addWidget(cbSweepType));

    auto eStart = new SIUnitEdit("Hz", " kMG", 6);
    // calculate width required with expected string length
    eStart->setMaxDecimals(2);
    Theme::fitWidth(eStart, "100.00 MHz");
    ...
    eCenter->setMaxDecimals(2);
    Theme::fitWidth(eCenter, "100.00 MHz");
    ...
    eStop->setMaxDecimals(2);
    Theme::fitWidth(eStop, "100.00 MHz");
    ...
    eSpan->setMaxDecimals(2);
    Theme::fitWidth(eSpan, "100.00 MHz");
    ...
    auto bFull = new QPushButton(QIcon::fromTheme("zoom-fit-best", QIcon(":/icons/zoom-fit.svg")), "");
    ...
    auto bZoomIn = new QPushButton(QIcon::fromTheme("zoom-in", QIcon(":/icons/zoom-in.svg")), "");
    ...
    auto bZoomOut = new QPushButton(QIcon::fromTheme("zoom-out", QIcon(":/icons/zoom-out.svg")), "");
    ...
    auto bAntenna = new QPushButton("Ant");
    bAntenna->setToolTip("Antenna band: sweep, VSWR limit and band marker");
    Theme::fitWidth(bAntenna, "Ant");
    connect(bAntenna, &QPushButton::clicked, this, &VNA::StartAntennaBandSetup);
    frequencySweepActions.push_back(tb_sweep->addWidget(bAntenna));

    auto bReport = new QPushButton("PDF");
    bReport->setToolTip("Antenna report (PDF)");
    Theme::fitWidth(bReport, "PDF");
    connect(bReport, &QPushButton::clicked, this, &VNA::StartAntennaReport);
    frequencySweepActions.push_back(tb_sweep->addWidget(bReport));

    auto bCable = new QPushButton("TDR");
    bCable->setToolTip("Cable impedance (TDR)");
    Theme::fitWidth(bCable, "TDR");
    connect(bCable, &QPushButton::clicked, this, &VNA::StartCableImpedanceSetup);
    frequencySweepActions.push_back(tb_sweep->addWidget(bCable));

    auto bLoss = new QPushButton("Loss");
    bLoss->setToolTip("Cable loss (far end open or shorted)");
    Theme::fitWidth(bLoss, "Loss");
    connect(bLoss, &QPushButton::clicked, this, &VNA::StartCableLossSetup);
    frequencySweepActions.push_back(tb_sweep->addWidget(bLoss));
    ...
    compactHiddenActions.push_back(frequencySweepActions.back());

    // power sweep widgets
    auto sbPowerLow = new QDoubleSpinBox();
    Theme::fitWidth(sbPowerLow, "-30.00dBm");
    ...
    Theme::fitWidth(sbPowerHigh, "-30.00dBm");
    ...
    Theme::fitWidth(ePowerFreq, "3.00000GHz");
    ...
    Theme::fitWidth(eBandwidth, "100.0 kHz");
    ...
    compactHiddenActions.push_back(tb_acq->addWidget(new QLabel("Dwell time:")));
    acquisitionDwellTime = new SIUnitEdit("s", "um ", 3);
    Theme::fitWidth(acquisitionDwellTime, "100 ms");
    ...
    compactHiddenActions.push_back(tb_acq->addWidget(acquisitionDwellTime));

    compactHiddenActions.push_back(tb_acq->addWidget(new QLabel("Averaging:")));
    lAverages = new QLabel("0/");
    compactHiddenActions.push_back(tb_acq->addWidget(lAverages));
    auto sbAverages = new QSpinBox;
    sbAverages->setRange(1, 99);
    Theme::fitWidth(sbAverages, "99");
    ...
    compactHiddenActions.push_back(tb_acq->addWidget(sbAverages));
    auto bResetAvg = new QPushButton("Reset");
    connect(bResetAvg, &QPushButton::clicked, this, [=](){
        average.reset(settings.npoints);
        UpdateAverageCount();
    });
    compactHiddenActions.push_back(tb_acq->addWidget(bResetAvg));
...
//        saveCal->setEnabled(false);
    ...
    auto configureToolbarForFrequencySweep = [=](){
        ...
        setCompactToolbar(Preferences::getInstance().UI.compactToolbar);
    };
...
void VNA::fromJSON(nlohmann::json j)
{
    ...
    if(j.contains("traces")) {
        ...
        // setups saved with the upstream default colors get the theme palette
        retintTraces(Theme::current(), Theme::current());
    }
    if(j.contains("tiles")) {
...
void VNA::StartAntennaBandSetup()
{
    auto &pref = Preferences::getInstance();
    AntennaBandDialog::Settings initial;
    initial.fLow = pref.UI.antennaBandStart;
    initial.fHigh = pref.UI.antennaBandStop;
    initial.vswr = pref.UI.antennaBandVSWR;
    initial.marginPercent = pref.UI.antennaBandMargin;
    initial.setSweep = true;
    initial.addLimitLine = true;
    initial.addMarker = true;
    initial.cableType = pref.UI.antennaCableType;
    initial.cableLength = pref.UI.antennaCableLength;
    initial.cableVF = pref.UI.antennaCableVF;
    initial.cableLoss = pref.UI.antennaCableLoss;

    auto dialog = new AntennaBandDialog(initial, window);
    connect(dialog, &QDialog::accepted, this, [=]() {
        auto s = dialog->settings();
        auto &pref = Preferences::getInstance();
        pref.UI.antennaBandStart = s.fLow;
        pref.UI.antennaBandStop = s.fHigh;
        pref.UI.antennaBandVSWR = s.vswr;
        pref.UI.antennaBandMargin = s.marginPercent;
        pref.UI.antennaCableType = s.cableType;
        pref.UI.antennaCableLength = s.cableLength;
        pref.UI.antennaCableVF = s.cableVF;
        pref.UI.antennaCableLoss = s.cableLoss;
        pref.store();

        const QString lineName = "Antenna VSWR " + QString::number(s.vswr, 'g', 3);

        // 0. cable between calibration plane and antenna: port extension with its delay and loss,
        //    so that VSWR/impedance refer to the antenna connector. Replaces the one from an earlier run.
        if(antennaCableExtension) {
            auto &options = deembedding.getOptions();
            auto it = std::find(options.begin(), options.end(), antennaCableExtension);
            if(it != options.end()) {
                deembedding.removeOption(it - options.begin());
            }
            antennaCableExtension = nullptr;
            antennaCableDescription.clear();
        }
        if(s.cableType != CableLibrary::noneName() && s.cableLength > 0) {
            double vf = s.cableVF;
            double lossAt1GHz = s.cableLoss;
            if(auto c = CableLibrary::find(s.cableType)) {
                vf = c->vf;
                lossAt1GHz = c->lossPerMeter(1e9);
            }
            if(vf < 0.05) {
                vf = 0.05;
            }
            const double c0 = 299792458.0;
            auto ext = new PortExtension();
            nlohmann::json jext;
            jext["port"] = 1;
            jext["delay"] = s.cableLength / (c0 * vf);
            jext["velocityFactor"] = vf;
            jext["DCloss"] = 0.0;
            jext["loss"] = lossAt1GHz * s.cableLength;
            jext["frequency"] = 1e9;
            ext->fromJSON(jext);
            deembedding.addOption(ext);   // enables de-embedding via optionAdded
            antennaCableExtension = ext;
            antennaCableDescription = s.cableType + ", " + Unit::ToString(s.cableLength, "m", "m k", 4)
                    + ": delay " + Unit::ToString(s.cableLength / (c0 * vf), "s", "pnum ", 3)
                    + ", loss " + QString::number(lossAt1GHz * s.cableLength, 'f', 2) + " dB @ 1 GHz (port extension)";
            connect(ext, &DeembeddingOption::deleted, this, [=](DeembeddingOption *o) {
                if(antennaCableExtension == o) {
                    antennaCableExtension = nullptr;
                    antennaCableDescription.clear();
                }
            });
        ...
        // 1. sweep around the band
        if(s.setSweep) {
            if(settings.sweepType != SweepType::Frequency) {
                SetSweepType(SweepType::Frequency);
            }
            settings.Freq.start = s.sweepStart();
            settings.Freq.stop = s.sweepStop();
            ConstrainAndUpdateFrequencies();
            WarnIfUncalibrated("Antenna band");
        }

        // 2. limit line: prefer a VSWR axis, fall back to a dB magnitude axis
        if(s.addLimitLine) {
            TraceXYPlot *target = nullptr;
            int targetAxis = 0;
            bool targetIsVSWR = false;
            for(auto p : TracePlot::getPlots()) {
                auto xy = dynamic_cast<TraceXYPlot*>(p);
                if(!xy || &xy->getModel() != &traceModel) {
                    continue;
                }
                for(int i=0;i<2;i++) {
                    ...
                    if(type == YAxis::Type::VSWR) {
                        ...
                        targetAxis = i;
                        targetIsVSWR = true;
                    } else if(type == YAxis::Type::Magnitude && !targetIsVSWR && !target) {
                        target = xy;
                        targetAxis = i;
                    }
                }
            ...
            if(target) {
                // replace a limit line from an earlier run instead of stacking them
                for(auto p : TracePlot::getPlots()) {
                    auto xy = dynamic_cast<TraceXYPlot*>(p);
                    if(xy && &xy->getModel() == &traceModel) {
                        xy->removeConstantLines(lineName);
                    }
                }
                double limit = s.vswr;
                if(!targetIsVSWR) {
                    limit = Util::SparamTodB((s.vswr - 1.0) / (s.vswr + 1.0));
                }
                auto line = new XYPlotConstantLine;
                line->setName(lineName);
                line->setColor(Theme::limitColor());
                line->setAxis(targetAxis == 0 ? XYPlotConstantLine::Axis::Primary : XYPlotConstantLine::Axis::Secondary);
                line->setPassFail(XYPlotConstantLine::PassFail::HighLimit);
                line->setPoints({QPointF(s.fLow, limit), QPointF(s.fHigh, limit)});
                target->addConstantLine(line);
            } else {
                InformationBox::ShowMessage("Antenna band", "No XY graph with a VSWR or magnitude axis found, limit line not added.");
            }
        }

        // 3. antenna band marker on the first live reflection trace
        if(s.addMarker) {
            Trace *trace = nullptr;
            for(auto t : traceModel.getTraces()) {
                if(t->isReflection() && t->getSource() == Trace::Source::Live) {
                    trace = t;
                    break;
                }
            }
            ...
            if(trace) {
                ...
                for(auto m : markerModel->getMarkers(trace)) {
                    if(m->getType() == Marker::Type::AntennaBand && !m->getParent()) {
                        marker = m;
                        break;
                    }
                }
                ...
                if(!marker) {
                    ...
                    marker->setType(Marker::Type::AntennaBand);
                    created = true;
                }
                marker->setVSWRThreshold(s.vswr);
                marker->setRestriction(true, s.sweepStart(), s.sweepStop());
                if(created) {
                    markerModel->addMarker(marker);
                }
            } else {
                InformationBox::ShowMessage("Antenna band", "No reflection trace (S11/S22) found, marker not added.");
            }
        }
    });
    if(AppWindow::showGUI()) {
...
void VNA::StartAntennaReport()
{
    auto &pref = Preferences::getInstance();
    auto dialog = new AntennaReportDialog(pref.UI.antennaReportName, window);
    connect(dialog, &QDialog::accepted, this, [=]() {
        // copy everything out of the dialog first: it deletes itself once closed
        const QString name = dialog->antennaName();
        const QString notes = dialog->notes();
        const auto opt = dialog->options();

        auto &pref = Preferences::getInstance();
        pref.UI.antennaReportName = name;

        QString dir = pref.UISettings.Paths.report;
        if(dir.isEmpty() || !QDir(dir).exists()) {
            dir = QDir::homePath();
        }
        QString base = name.isEmpty() ? QString("antenna") : name;
        base.replace(QRegularExpression("[^A-Za-z0-9_\\-]+"), "_");
        auto now = QDateTime::currentDateTime();
        QString suggested = dir + "/" + base + "_" + now.toString("yyyyMMdd_HHmm") + ".pdf";
        auto filename = QFileDialog::getSaveFileName(window, "Save antenna report", suggested,
                                                     "PDF files (*.pdf)", nullptr, Preferences::QFileDialogOptions());
        if(filename.isEmpty()) {
            return;
        }
        ...
        pref.UISettings.Paths.report = QFileInfo(filename).path();
        pref.store();

        AntennaReport::Meta meta;
        meta.antennaName = name;
        meta.notes = notes;
        meta.time = now;
        auto dev = window->getDevice();
        if(dev) {
            auto info = DeviceDriver::getInfo(dev);
            meta.device = dev->getSerial() + ", HW " + info.hardware_version + ", FW " + info.firmware_version;
        } else {
            meta.device = "not connected";
        }
        if(cal.getCaltype().type != Calibration::Type::None) {
            QFileInfo fi(cal.getCurrentCalibrationFile());
            auto calfile = fi.fileName();
            if(calfile.isEmpty()) {
                calfile = "unsaved";
            }
            meta.calibration = cal.getCaltype().getShortString() + ", " + calfile;
        } else {
            meta.calibration = "none";
        }
        if(antennaCableExtension && deembedding_active) {
            meta.cable = antennaCableDescription;
        } else if(deembedding_active) {
            meta.cable = "de-embedding active (manual setup)";
        }
        meta.sweep = Unit::ToString(settings.Freq.start, "Hz", " kMG", 6, 3) + " – "
                + Unit::ToString(settings.Freq.stop, "Hz", " kMG", 6, 3)
                + ", " + QString::number(settings.npoints) + " points"
                + ", IF BW " + Unit::ToString(settings.bandwidth, "Hz", " kM", 3)
                + ", averaging " + QString::number(averages)
                + ", level " + Unit::ToString(settings.Freq.excitation_power, "dBm", " ", 3);

        QStringList written;
        QString error;
        if(AntennaReport::write(filename, meta, opt, traceModel, *markerModel, written, error)) {
            InformationBox::ShowMessage("Antenna report", "Saved:\n" + written.join("\n"));
        } else {
            InformationBox::ShowError("Antenna report", error);
        }
    });
    if(AppWindow::showGUI()) {
        dialog->show();
...
void VNA::StartCableImpedanceSetup()
{
    auto &pref = Preferences::getInstance();
    CableImpedanceDialog::Settings initial;
    initial.knownLength = pref.UI.cableKnownLength;
    initial.length = pref.UI.cableLength;
    initial.velocityFactor = pref.UI.cableVelocityFactor;
    initial.zTolerance = pref.UI.cableZTolerance;
    initial.cableType = pref.UI.cableType;
    initial.nominalVF = 0.0;
    initial.setSweep = true;
    initial.createTrace = true;
    initial.addMarker = true;

    auto dialog = new CableImpedanceDialog(initial, window);
    connect(dialog, &QDialog::accepted, this, [=]() {
        auto s = dialog->settings();
        auto &pref = Preferences::getInstance();
        pref.UI.cableKnownLength = s.knownLength;
        pref.UI.cableLength = s.length;
        pref.UI.cableVelocityFactor = s.velocityFactor;
        pref.UI.cableZTolerance = s.zTolerance;
        pref.UI.cableType = s.cableType;
        pref.store();

        // 1. TDR-friendly sweep: evenly spaced points from f_step up to the maximum frequency
        if(s.setSweep) {
            if(settings.sweepType != SweepType::Frequency) {
                SetSweepType(SweepType::Frequency);
            }
            ...
            double stop = DeviceDriver::getInfo(window->getDevice()).Limits.VNA.maxFreq;
            if(stop <= 0) {
                stop = pref.UI.maxFrequency;
            }
            double step = stop / points;
            if(step < DeviceDriver::getInfo(window->getDevice()).Limits.VNA.minFreq) {
                step = DeviceDriver::getInfo(window->getDevice()).Limits.VNA.minFreq;
            }
            settings.Freq.start = step;
            settings.Freq.stop = stop;
            ConstrainAndUpdateFrequencies();
            SetPoints(points);
            WarnIfUncalibrated("Cable impedance");
        }

        // 2. trace with TDR step response, reuse an existing time domain reflection trace if there is one
        Trace *trace = nullptr;
        for(auto t : traceModel.getTraces()) {
            if(t->isReflection() && t->outputType() == Trace::DataType::Time && t->getSource() == Trace::Source::Live) {
                trace = t;
                break;
            }
        }
        if(!trace && s.createTrace) {
            trace = new Trace("S11 TDR", Theme::traceColor(9), "S11");
            auto tdr = new Math::TDR();
            nlohmann::json jtdr;
            jtdr["bandpass_mode"] = false;
            jtdr["step_response"] = true;
            jtdr["automatic_DC"] = true;
            jtdr["padding"] = 0;
            tdr->fromJSON(jtdr);
            trace->addMathOperation(tdr);
            trace->enableMath(true);
            traceModel.addTrace(trace);

            auto plot = new TraceXYPlot(traceModel);
            plot->setXAxis(XAxis::Type::Distance, TraceXYPlot::XAxisMode::FitTraces, false, 0, 1, 10, true);
            // fixed 0..100 Ohm: with autorange the open end (Z -> infinity) squeezes the cable plateau to zero
            plot->setYAxis(0, YAxis::Type::Impedance, false, false, 0, 100, 10, true);
            plot->setYAxis(1, YAxis::Type::Disabled, false, false, 0, 1, 1, true);
            plot->enableTrace(trace, true);
            tiles->addPlotInNewTile(plot);
        }
        if(!trace) {
            InformationBox::ShowMessage("Cable impedance", "No time domain reflection trace found and trace creation disabled, marker not added.");
            return;
        }
        if(!s.knownLength) {
            trace->setVelocityFactor(s.velocityFactor);
        }

        // 3. cable impedance marker
        if(s.addMarker) {
            Marker *marker = nullptr;
            for(auto m : markerModel->getMarkers(trace)) {
                if(m->getType() == Marker::Type::CableImpedance && !m->getParent()) {
                    marker = m;
                    break;
                }
            }
            ...
            if(!marker) {
                marker = markerModel->createDefaultMarker();
                marker->assignTrace(trace);
                marker->setType(Marker::Type::CableImpedance);
                created = true;
            }
            marker->setZTolerance(s.zTolerance);
            marker->setKnownLength(s.knownLength ? s.length : 0.0);
            marker->setNominalVF(s.nominalVF);
            marker->resetSection();
            if(created) {
                markerModel->addMarker(marker);
            }
...
void VNA::StartCableLossSetup()
{
    auto &pref = Preferences::getInstance();
    CableLossDialog::Settings initial;
    initial.cableType = pref.UI.cableLossType;
    initial.length = pref.UI.cableLossLength;
    initial.fLow = pref.UI.cableLossStart;
    initial.fHigh = pref.UI.cableLossStop;
    initial.setSweep = true;
    initial.createTrace = true;
    initial.addMarker = true;

    const QString lossTraceName = "Cable loss";
    auto dialog = new CableLossDialog(initial, window);
    connect(dialog, &QDialog::accepted, this, [=]() {
        auto s = dialog->settings();
        auto &pref = Preferences::getInstance();
        pref.UI.cableLossType = s.cableType;
        pref.UI.cableLossLength = s.length;
        pref.UI.cableLossStart = s.fLow;
        pref.UI.cableLossStop = s.fHigh;
        pref.store();

        // 1. sweep over the range of interest
        if(s.setSweep) {
            if(settings.sweepType != SweepType::Frequency) {
                SetSweepType(SweepType::Frequency);
            }
            settings.Freq.start = s.fLow;
            settings.Freq.stop = s.fHigh;
            ConstrainAndUpdateFrequencies();
            WarnIfUncalibrated("Cable loss");
        }

        // 2. the trace the marker works on: the plain (live) reflection measurement. The loss
        //    trace below is derived from S11 as well, so it has to be excluded here.
        Trace *loss = nullptr;
        for(auto t : traceModel.getTraces()) {
            if(t->name() == lossTraceName) {
                loss = t;
                break;
            }
        }
        Trace *reflection = nullptr;
        for(int pass = 0;pass < 2 && !reflection;pass++) {
            for(auto t : traceModel.getTraces()) {
                if(t == loss || !t->isReflection() || t->getSource() != Trace::Source::Live
                        || t->outputType() != Trace::DataType::Frequency) {
                    continue;
                }
                // prefer an untouched measurement, accept one with math operations otherwise
                if(pass == 0 && t->hasMathOperations()) {
                    continue;
                }
                reflection = t;
                break;
        ...
        if(!reflection) {
            InformationBox::ShowMessage("Cable loss", "No live reflection trace (S11/S22) found, nothing to measure.");
            return;
        }

        // 3. trace showing the one way loss: |1/sqrt(S11)| in dB is -|S11|dB / 2, so a magnitude
        //    graph of it reads the cable loss directly in positive dB
        if(s.createTrace) {
            if(!loss) {
                loss = new Trace(lossTraceName, Theme::traceColor(4), reflection->liveParameter());
                auto exp = new Math::Expression();
                nlohmann::json jexp;
                jexp["exp"] = "1/sqrt(x)";
                exp->fromJSON(jexp);
                loss->addMathOperation(exp);
                loss->enableMath(true);
                traceModel.addTrace(loss);

                auto plot = new TraceXYPlot(traceModel);
                plot->setXAxis(XAxis::Type::Frequency, TraceXYPlot::XAxisMode::UseSpan, false, 0, 1, 10, true);
                plot->setYAxis(0, YAxis::Type::Magnitude, false, true, 0, 10, 10, true);
                plot->setYAxis(1, YAxis::Type::Disabled, false, false, 0, 1, 1, true);
                plot->enableTrace(loss, true);
                tiles->addPlotInNewTile(plot);
            }
        }

        // 4. cable loss marker on the reflection trace
        if(s.addMarker) {
            Marker *marker = nullptr;
            for(auto m : markerModel->getMarkers(reflection)) {
                if(m->getType() == Marker::Type::CableLoss && !m->getParent()) {
                    marker = m;
                    break;
                }
            }
            ...
            if(!marker) {
                marker = markerModel->createDefaultMarker();
                marker->assignTrace(reflection);
                marker->setType(Marker::Type::CableLoss);
                created = true;
            }
            marker->setRestriction(true, s.fLow, s.fHigh);
            marker->setKnownLength(s.length);
            marker->setCableType(s.cableType);
            if(created) {
                markerModel->addMarker(marker);
            }
        }
...
void VNA::WarnIfUncalibrated(const QString &title)
{
    switch(getCalInterpolation()) {
    case Calibration::InterpolationType::NoCalibration:
        InformationBox::ShowMessage(title, "No calibration is active. S11 and everything derived from it (VSWR, impedance, TDR) "
                                    "is raw receiver data. Load or perform an OSL calibration covering this sweep.");
        break;
    case Calibration::InterpolationType::Extrapolate:
        InformationBox::ShowMessage(title, "The sweep extends beyond the calibrated frequency range, points outside "
                                    "of it are extrapolated. Consider a calibration covering the whole sweep.");
        break;
    default:
        break;
    }
...
void VNA::retintTraces(Theme::Kind from, Theme::Kind to)
{
    for(auto t : traceModel.getTraces()) {
        auto c = Theme::remapTraceColor(t->color(), from, to);
        if(c != t->color()) {
            t->setColor(c);
        }
    }
...
void VNA::createDefaultTracesAndGraphs(int ports)
{
    auto getDefaultColor = [](int ports, int i, int j)->QColor {
        // Theme palette, indexed as in a 4-port matrix so that e.g. S21 always has the same color
        if(ports >= 1 && ports <= 4) {
            return Theme::traceColor(i*4+j);
        } else {
            return Theme::traceColor(i*ports+j);
        }
    };
...
void VNA::setCompactToolbar(bool compact)
{
    for(auto a : compactHiddenActions) {
        a->setVisible(!compact);
    }
}
