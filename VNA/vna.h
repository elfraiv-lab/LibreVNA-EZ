...
private slots:
    ...
    void StartAntennaBandSetup();
    void StartAntennaReport();
    void StartCableImpedanceSetup();
    void StartCableLossSetup();
    // shows a warning when the current sweep is not (fully) covered by the active calibration
    void WarnIfUncalibrated(const QString &title);
    // Sweep control
...
public:
    // hides/shows the toolbar items that are not needed for plain S11/VSWR work
    void setCompactToolbar(bool compact);
    // traces still using the default palette of theme `from` get the matching color of `to`
    void retintTraces(Theme::Kind from, Theme::Kind to);
private:
    Settings settings;
    ...
    std::vector<QAction*> compactHiddenActions; // toolbar entries hidden in compact mode
    ...
    DeembeddingOption *antennaCableExtension;   // port extension created by the antenna band dialog (or nullptr)
    QString antennaCableDescription;            // human readable description of that extension for the report
    ...
    TileWidget *tiles;
    UndoHistory *undoHistory = nullptr; // snapshot undo/redo of the setup (Edit toolbar, Ctrl+Z / Ctrl+Y)
    QScrollArea *central;
