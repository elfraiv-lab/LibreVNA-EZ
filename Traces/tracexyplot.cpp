...
void TraceXYPlot::setYAxis(int axis, YAxis::Type type, bool log, bool autorange, double min, double max, unsigned int divs, bool autoDivs)
{
    ...
    if(type == YAxis::Type::VSWR) {
        // VSWR axis is limited to 1..20 (setup dialog / loaded setup)
        constrainYRange(type, min, max);
    }
...
bool TraceXYPlot::xAxisNonNegative()
{
...
// Keeps a manually panned/zoomed X range inside the physically meaningful
// window: frequency 100 kHz..6 GHz (device range), time/distance >= 0.
void TraceXYPlot::constrainXRange(double &min, double &max)
{
    if(!xAxisNonNegative()) {
        return;
    }
    const bool freq = (xAxis.getType() == XAxis::Type::Frequency);
    auto &pref = Preferences::getInstance();
    const double lower = freq ? pref.UI.minFrequency : 0.0;
    const double upper = freq ? pref.UI.maxFrequency : std::numeric_limits<double>::max();
    double span = max - min;
    if(span > upper - lower) {
        span = upper - lower;
    }
...
// VSWR axis is kept within 1..20 (autorange, manual setup, pan and zoom).
// The visible span is preserved where possible, shifting the window back inside the limits.
void TraceXYPlot::constrainYRange(YAxis::Type type, double &min, double &max)
{
    if(type != YAxis::Type::VSWR) {
        return;
    }
    const double lower = 1.0;
    const double upper = 20.0;
    double span = max - min;
    if(span > upper - lower) {
        span = upper - lower;
    }
...
void TraceXYPlot::constrainYRange(const YAxis &axis, double &min, double &max)
{
    constrainYRange(axis.getType(), min, max);
}
...
void TraceXYPlot::move(const QPoint &vect)
{
    if(!xAxis.getLog()) {
        ...
        double min = xAxis.getRangeMin() - distance;
        double max = xAxis.getRangeMax() - distance;
        constrainXRange(min, max);
    ...
    for(int i=0;i<2;i++) {
        if(!yAxis[i].getLog()) {
            ...
            double min = yAxis[i].getRangeMin() - distance;
            double max = yAxis[i].getRangeMax() - distance;
            constrainYRange(yAxis[i], min, max);
...
void TraceXYPlot::addConstantLine(XYPlotConstantLine *line)
{
    constantLines.push_back(line);
    replot();
}

int TraceXYPlot::removeConstantLines(const QString &name)
{
    int removed = 0;
    for(auto it = constantLines.begin(); it != constantLines.end();) {
        if((*it)->getName() == name) {
            delete *it;
            it = constantLines.erase(it);
            removed++;
        } else {
            ++it;
        }
...
{
    if(YAxisEnabled) {
        // rotated axis name + widest expected tick label ("-00.00 dBm") in the readout font + gaps
        auto fs = Theme::graphMetrics().fontSizeAxis;
        QFontMetrics fm(Theme::graphFont(fs));
        return fs * 1.5 + fm.horizontalAdvance("-00.00 dBm") + 8;
    } else {
...
void TraceXYPlot::draw(QPainter &p)
{
    ...
    p.setPen(pen);
    auto yAxisSpace = sideMargin(true);
    auto xAxisSpace = gm.fontSizeAxis * 3;
    ...
    // draw axis types
    auto font = Theme::graphFont(gm.fontSizeAxis);
    p.setFont(font);
    p.drawText(QRect(0, w.height()-gm.fontSizeAxis*1.5, w.width(), gm.fontSizeAxis*1.5), Qt::AlignHCenter, xAxis.TypeToName());
    for(int i=0;i<2;i++) {
        ...
        auto xStart = i == 0 ? 0 : w.width() - gm.fontSizeAxis * 1.5;
        p.save();
        p.translate(xStart, w.height()-xAxisSpace);
        p.rotate(-90);
        p.drawText(QRect(0, 0, w.height()-xAxisSpace, gm.fontSizeAxis*1.5), Qt::AlignHCenter, labelY);
        p.restore();
        // draw ticks
        if(yAxis[i].getType() != YAxis::Type::Disabled && yAxis[i].getTicks().size() > 0) {
            ...
            for(unsigned int j = 0; j < yAxis[i].getTicks().size(); j++) {
                ...
                if(yCoord + gm.fontSizeAxis >= lastTickLabelEnd) {
                    ...
                    QRect bounding;
                    if(i == 0) {
                        p.drawText(QRect(0, yCoord - gm.fontSizeAxis/2 - 2, tickStart + 2 * tickLen, gm.fontSizeAxis*1.5), Qt::AlignRight, tickValue, &bounding);
                    } else {
                        p.drawText(QRect(tickStart + 2 * tickLen + 2, yCoord - gm.fontSizeAxis/2 - 2, yAxisSpace, gm.fontSizeAxis*1.5), Qt::AlignLeft, tickValue, &bounding);
                    }
        ...
        for(auto t : tracesAxis[i]) {
            ...
            if(i == 0 && nPoints > 0) {
                ...
                for(auto m : markers) {
                    ...
                    // marker lines (VSWR threshold, TDR section) in the plot foreground, dashed,
                    // so they do not merge with the trace they belong to
                    auto markerPen = QPen(Theme::graphTextColor(), gm.lineWidth, Qt::DashLine);
        ...
        for(auto line : constantLines) {
            ...
            pen = QPen(line->getColor(), gm.lineWidth);
            pen.setCosmetic(true);
            if(i == 1) {
                pen.setStyle(Qt::DotLine);
            } else {
    ...
    if(xAxis.getTicks().size() >= 1) {
        ...
        if(!displayFullFreq) {
            ...
            QRect bounding;
            p.drawText(QRect(2, plotAreaBottom + gm.fontSizeAxis + 5, w.width(), gm.fontSizeAxis*1.5), 0, front, &bounding);
            p.setPen(pref.Graphs.Color.axis);
            p.drawText(QRect(bounding.x() + bounding.width(), plotAreaBottom + gm.fontSizeAxis + 5, w.width(), gm.fontSizeAxis*1.5), 0, back);
        }
        ...
        for(auto t : xAxis.getTicks()) {
            ...
            if(displayFullFreq) {
                QRect bounding;
                p.drawText(QRect(xCoord - gm.fontSizeAxis*4, plotAreaBottom + 5, gm.fontSizeAxis*8,
                                 gm.fontSizeAxis*1.5), Qt::AlignHCenter, tickValue, &bounding);
                ...
                QRect bounding;
                p.drawText(QRect(xCoord - gm.fontSizeAxis*4, plotAreaBottom + 5, gm.fontSizeAxis*8, gm.fontSizeAxis*1.5), Qt::AlignHCenter, tickValue, &bounding);
                lastTickLabelEnd = bounding.x() + bounding.width();
                p.setPen(QPen(QColor("orange")));
                p.drawText(QRect(0, plotAreaBottom + 5, bounding.x() - 1, gm.fontSizeAxis*1.5), Qt::AlignRight, "..", &bounding);
            }
        }
    }
    ...
    if((xAxis.getType() == XAxis::Type::Frequency || xAxis.getType() == XAxis::Type::TimeZeroSpan || xAxis.getType() == XAxis::Type::Power)
        ...
        if(xSweep >= xAxis.getRangeMin() && xSweep <= xAxis.getRangeMax()) {
            ...
            if(pref.Graphs.SweepIndicator.triangle) {
                for(int i=0;i<gm.triangleSize;i++) {
...
void TraceXYPlot::updateAxisTicks()
{
    if(xAxisMode != XAxisMode::Manual) {
        ...
        if(xAxisMode == XAxisMode::UseSpan) {
            ...
            if((xAxis.getType() == XAxis::Type::Time || xAxis.getType() == XAxis::Type::Distance) && min < 0.0 && max > 0.0) {
                // TDR output is symmetric around t = 0, the acausal half carries no information: start at 0
                min = 0.0;
            }
        }
