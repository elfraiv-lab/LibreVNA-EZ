...
//    p.setRenderHint(QPainter::Antialiasing);
    ...
    if(!title.isEmpty()) {
        QFont font = p.font();
        font.setPixelSize(gm.fontSizeTitle);
        p.setFont(font);
        p.setPen(Util::getFontColorFromBackground(pref.Graphs.Color.background));
        p.drawText(QRect(0, 0, width(), gm.fontSizeTitle), Qt::AlignCenter, title);
        marginTop += gm.fontSizeTitle;
    }

    // show names of active traces and marker data (if enabled)
    bool hasMarkerData = false;
    // width of the marker data column: default from the font size, widened when a
    // marker line (e.g. antenna band or cable impedance) needs more, capped at 45%
    double marginMarkerData = gm.fontSizeMarkerData * 12.5;
    if(showMarkerData) {
        QFont mdFont = Theme::graphFont(gm.fontSizeMarkerData);
        QFontMetrics mdMetrics(mdFont);
        int needed = 0;
        for(auto t : traces) {
            if(!t.second || !t.first->isVisible()) {
                continue;
            }
            for(auto m : t.first->getMarkers()) {
                if(!m->isVisible() || !markerVisible(m->getPosition()) || m->getGraphDisplayFormats().size() == 0) {
                    continue;
                }
                auto header = mdMetrics.horizontalAdvance("  " + QString::number(m->getNumber()) + "  ") + 5
                        + mdMetrics.horizontalAdvance(m->getSuffix() + "  " + m->readablePosition());
                needed = std::max(needed, header);
                for(auto f : m->getGraphDisplayFormats()) {
                    needed = std::max(needed, mdMetrics.horizontalAdvance(m->readableData(f)));
                }
            }
        }
        needed += 10;
        double cap = width() * 0.45;
        if(needed > marginMarkerData) {
            marginMarkerData = std::min((double) needed, std::max(cap, marginMarkerData));
        }
    }
    auto traceNameTop = marginTop + 5;
    marginTop += gm.fontSizeTraceNames + 8;
    ...
    for(auto t : traces) {
        ...
        // Trace name
        auto textArea = QRect(x, traceNameTop, width() - x, gm.fontSizeTraceNames + 8);
        QFont font = this->font();
        font.setPixelSize(gm.fontSizeTraceNames);
        p.setFont(font);
        ...
        p.setPen(Theme::readableOn(t.first->color(), pref.Graphs.Color.background));
        ...
        for(auto m : vmarkers) {
            ...
            hasMarkerData = true;
            QFont font = Theme::graphFont(gm.fontSizeMarkerData);
            p.setFont(font);
    ...
    int mdw = hasMarkerData ? (int) marginMarkerData : 0;
    if(mdw != markerDataWidth) {
        markerDataWidth = mdw;
        emit tileFitChanged();
    }

    // signed arithmetic: a plot narrower than its margins (e.g. while the layout
    // settles) must not wrap around into a huge/garbage size. Qt 6.11 QRect
    // asserts on integer overflow, so skip drawing a degenerate area instead.
    int l = marginLeft;
    int t = marginTop;
    int w = (int) width() - (int) marginLeft - (int) marginRight - mdw;
    int h = (int) height() - (int) marginTop - (int) marginBottom;
    if(w < 1 || h < 1) {
        return;
    }
...
void TracePlot::finishContextMenu()
{
    ...
    auto markerData = new QAction("Show marker data", contextmenu);
    markerData->setCheckable(true);
    markerData->setChecked(showMarkerData);
    contextmenu->addAction(markerData);
    connect(markerData, &QAction::toggled, this, [=](bool on){
        showMarkerData = on;
        triggerReplot();
    });
    contextmenu->addSeparator();
...
void TracePlot::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(selectedMarker) {
        ...
        if(!text.isEmpty()) {
            ...
            auto font = Theme::graphFont(Theme::graphMetrics().fontSizeCursorOverlay);
...
nlohmann::json TracePlot::getBaseJSON()
{
    ...
    j["showMarkerData"] = showMarkerData;
    return j;
}
...
void TracePlot::parseBaseJSON(nlohmann::json j)
{
    ...
    showMarkerData = j.value("showMarkerData", true);
}
