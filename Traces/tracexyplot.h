...
public:
    ...
    // programmatic setup (used by the antenna band quick setup)
    QString getName() const { return name; }
    PassFail getPassFail() const { return passFail; }
    void setName(const QString &value) { name = value; }
    void setAxis(Axis value) { axis = value; }
    void setPassFail(PassFail value) { passFail = value; }
    void setPoints(const std::vector<QPointF> &value) { points = value; }
...
public:
    ...
    bool xAxisNonNegative();
    void constrainXRange(double &min, double &max);
    void constrainYRange(YAxis::Type type, double &min, double &max);
    void constrainYRange(const YAxis &axis, double &min, double &max);
    ...
    YAxis::Type getYAxisType(int axis) const { return yAxis[axis].getType(); }
    // takes ownership of the line
    void addConstantLine(XYPlotConstantLine *line);
    // removes (and deletes) all constant lines with the given name, returns number removed
    int removeConstantLines(const QString &name);
    bool hasLimitLines() const { return !constantLines.empty(); }
    const std::vector<XYPlotConstantLine*>& getConstantLines() const { return constantLines; }
