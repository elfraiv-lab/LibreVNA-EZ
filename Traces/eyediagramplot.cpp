...
void EyeDiagramPlot::draw(QPainter &p)
{
    ...
    auto w = p.window();
    auto yAxisSpace = gm.fontSizeAxis * 5.5;
    ...
    if(yAxis.getTicks().size() > 0) {
        ...
        for(unsigned int j = 0; j < yAxis.getTicks().size(); j++) {
            ...
            p.drawText(QRectF(0, yCoord - gm.fontSizeAxis/2 - 2, tickStart + 2 * tickLen, gm.fontSizeAxis*1.5), Qt::AlignRight, tickValue);
    ...
    // X axis name
    p.setPen(QPen(pref.Graphs.Color.axis, 1));
    p.drawText(QRect(plotAreaLeft, w.height()-gm.fontSizeAxis*1.5, plotAreaWidth, gm.fontSizeAxis*1.5), Qt::AlignHCenter, "Time");
