...
Marker::Marker(MarkerModel *model, int number, Marker *parent, QString descr)
    : editingFrequency(false),
      ...
      vswrThreshold(2.0),
      bandFound(false),
      bandLowClipped(false),
      bandHighClipped(false),
      zTolerance(5.0),
      sectionManual(false),
      sectionUpdating(false),
      knownLength(0.0),
      nominalVF(0.0),
      cableValid(false),
      cableClipped(false),
...
QString Marker::formatToString(Marker::Format f)
{
    ...
    case Format::BandEdges: return "Band edges (VSWR)";
    case Format::Bandwidth: return "Bandwidth (VSWR)";
    case Format::StatVSWR: return "VSWR min/max/mean";
    case Format::StatMagnitude: return "|S| min/max/mean";
    case Format::Ripple: return "Ripple (p-p)";
    case Format::CableImpedance: return "Cable impedance";
    case Format::CableLength: return "Cable length";
    case Format::CableDelay: return "Cable delay / VF";
    case Format::CableLoss: return "Cable loss";
    case Format::CableLossBand: return "Cable loss min/max/mean";
    case Format::CableLossPerLength: return "Cable loss per length";
...
std::vector<Marker::Format> Marker::applicableFormats()
{
    ...
    case Trace::DataType::Time:
        ...
        case Type::Delta:
            ...
            break;
        case Type::CableImpedance:
            ret.push_back(Format::CableImpedance);
            ret.push_back(Format::CableLength);
            ret.push_back(Format::CableDelay);
            ret.push_back(Format::Impedance);
            break;
        default:
            break;
    ...
    case Trace::DataType::Frequency:
        ...
        case Type::Statistics:
            // marker sits at the worst point of the band
            if(parentTrace && parentTrace->isReflection()) {
                ret.push_back(Format::StatVSWR);
                ret.push_back(Format::StatMagnitude);
                ret.push_back(Format::Ripple);
                ret.push_back(Format::VSWR);
                ret.push_back(Format::dB);
                ret.push_back(Format::Impedance);
            } else {
            ...
            break;
        case Type::CableLoss:
            // marker sits at the worst case (highest loss) point of the band
            ret.push_back(Format::CableLoss);
            ret.push_back(Format::CableLossBand);
            ret.push_back(Format::CableLossPerLength);
            ret.push_back(Format::Ripple);
            ret.push_back(Format::dB);
            ret.push_back(Format::VSWR);
            break;
        case Type::AntennaBand:
            // marker sits at the resonance, so the usual reflection formats apply as well
            ret.push_back(Format::Bandwidth);
            ret.push_back(Format::BandEdges);
            ret.push_back(Format::VSWR);
            ret.push_back(Format::dB);
            ret.push_back(Format::Impedance);
            ret.push_back(Format::SeriesR);
    ...
    case Trace::DataType::Time:
        if(type != Type::Delta) {
            ...
            case Format::Impedance: {
                ...
                break;
            case Format::CableImpedance:
                if(type != Type::CableImpedance) {
                    return "Invalid";
                }
                if(!cableValid) {
                    return "Z: no flat section (TDR step response required)";
                }
                return "Z: " + QString::number(cableMeanZ, 'f', 1) + " Ω (min " + QString::number(cableMinZ, 'f', 1)
                        + ", max " + QString::number(cableMaxZ, 'f', 1) + ")";
            case Format::CableLength: {
                if(type != Type::CableImpedance) {
                    return "Invalid";
                }
                if(!cableValid) {
                ...
                if(knownLength > 0) {
                    return "L: " + Unit::ToString(knownLength, "m", "m k", 4) + " (set)";
                }
                double length = parentTrace->timeToDistance(cableEndTime) - parentTrace->timeToDistance(cableStartTime);
                return "L: " + QString(cableClipped ? ">" : "") + Unit::ToString(length, "m", "m k", 4)
                        + " (VF " + QString::number(parentTrace->velocityFactor(), 'f', 2) + ")";
            }
            case Format::CableDelay: {
                if(type != Type::CableImpedance) {
                    return "Invalid";
                }
                if(!cableValid) {
                ...
                double roundTrip = cableEndTime - cableStartTime;
                QString prefix = cableClipped ? ">" : "";
                QString ret = "τ: " + prefix + Unit::ToString(roundTrip / 2, "s", "pnum ", 3);
                if(knownLength > 0 && roundTrip > 0) {
                    double vf = knownLength / (c0 * roundTrip / 2);
                    ret += ", VF: " + QString(cableClipped ? "<" : "") + QString::number(vf, 'f', 3);
                    if(nominalVF > 0) {
                        ret += " (nom. " + QString::number(nominalVF, 'f', 3) + ")";
                    }
                }
                if(!cableClipped) {
                    // what follows the section tells whether the far end is open, shorted or terminated;
                    // an open cable showing "short end" means the phase is off (typically no calibration)
                    if(cableEndKind > 0) {
                        ret += ", open end";
                    } else if(cableEndKind < 0) {
                        ret += ", short end";
                    } else {
                        ret += " (end?)";
                    }
    ...
    case Trace::DataType::TimeZeroSpan:
        ...
        default:
            ...
            case Format::maxDeltaPos:
                ...
                break;
            case Format::StatVSWR: {
                if(type != Type::Statistics || !stats.valid) {
                    return "VSWR min/max/mean: n/a";
                }
                return "VSWR min/max/mean: " + QString::number(stats.vswrMin, 'f', 2) + " / "
                        + QString::number(stats.vswrMax, 'f', 2) + " / " + QString::number(stats.vswrMean, 'f', 2);
            }
            case Format::StatMagnitude: {
                if(type != Type::Statistics || !stats.valid) {
                    return "|S| min/max/mean: n/a";
                }
                return "|S| min/max/mean: " + QString::number(stats.magMin, 'f', 2) + " / "
                        + QString::number(stats.magMax, 'f', 2) + " / " + QString::number(stats.magMean, 'f', 2) + " dB";
            }
            case Format::Ripple: {
                if((type != Type::Statistics && type != Type::CableLoss) || !stats.valid) {
                    return "Ripple: n/a";
                }
                QString ret = "Ripple: " + QString::number(stats.ripplePP, 'f', 2) + " dB p-p";
                if(hasRippleLimit()) {
                    ret += " (limit " + QString::number(rippleLimit, 'f', 2) + ") ";
                    ret += isRipplePass() ? "PASS" : "FAIL";
                }
                return ret;
            }
            case Format::CableLoss: {
                if(type != Type::CableLoss || !stats.valid) {
                    return "Loss: n/a";
                }
                QString ret = "Loss: " + QString::number(stats.lossMax, 'f', 2) + " dB @ "
                        + Unit::ToString(position, "Hz", " kMG", 4) + " (one way)";
                if(stats.lossSuspicious) {
                    ret += " ?";
                }
                return ret;
            }
            case Format::CableLossBand: {
                if(type != Type::CableLoss || !stats.valid) {
                    return "Loss min/max/mean: n/a";
                }
                return "Loss min/max/mean: " + QString::number(stats.lossMin, 'f', 2) + " / "
                        + QString::number(stats.lossMax, 'f', 2) + " / "
                        + QString::number(stats.lossMean, 'f', 2) + " dB";
            }
            case Format::CableLossPerLength: {
                if(type != Type::CableLoss || !stats.valid) {
                    return "Loss/length: n/a";
                }
                if(knownLength <= 0) {
                    return "Loss/length: cable length unknown";
                }
                double perMeter = stats.lossMax / knownLength;
                QString ret = "Loss: " + QString::number(perMeter, 'f', 3) + " dB/m ("
                        + QString::number(perMeter * 100, 'f', 1) + " dB/100 m)";
                if(auto c = CableLibrary::find(cableTypeName)) {
                    ret += " (nom. " + QString::number(c->lossPerMeter(position), 'f', 3) + ")";
                }
                return ret;
            }
            case Format::BandEdges: {
                if(type != Type::AntennaBand || helperMarkers.size() < 2) {
                    return "Invalid";
                }
                auto thr = QString::number(vswrThreshold, 'g', 3);
                if(!bandFound) {
                    return "VSWR > " + thr + " at resonance";
                }
                QString ret = "fL: ";
                ret += (bandLowClipped ? "<" : "") + Unit::ToString(helperMarkers[0]->position, "Hz", " kMG", 5);
                ret += ", fH: ";
                ret += (bandHighClipped ? ">" : "") + Unit::ToString(helperMarkers[1]->position, "Hz", " kMG", 5);
                return ret;
            }
            case Format::Bandwidth: {
                if(type != Type::AntennaBand || helperMarkers.size() < 2) {
                    return "Invalid";
                }
                auto thr = QString::number(vswrThreshold, 'g', 3);
                if(!bandFound) {
                    return "BW(VSWR≤" + thr + "): none";
                }
                auto low = helperMarkers[0]->position;
                auto high = helperMarkers[1]->position;
                auto bandwidth = high - low;
                auto center = (high + low) / 2;
                QString ret = "BW(VSWR≤" + thr + "): ";
                if(bandLowClipped || bandHighClipped) {
                    ret += ">";
                }
                ret += Unit::ToString(bandwidth, "Hz", " kMG", 4);
                if(center > 0) {
                    ret += " (" + QString::number(bandwidth / center * 100.0, 'f', 1) + "%)";
                }
                return ret;
            }
...
QString Marker::readableSettings()
{
    ...
    case Trace::DataType::Time:
        ...
        case Type::CableImpedance:
            return "±" + Unit::ToString(zTolerance, "Ω", " ", 3);
        default:
    ...
    case Trace::DataType::Frequency:
        ...
        case Type::Flatness:
            return "None";
        case Type::AntennaBand:
            return "VSWR " + QString::number(vswrThreshold, 'g', 3) + ":1";
        case Type::Statistics:
            return hasRippleLimit() ? "ripple ≤ " + Unit::ToString(rippleLimit, "dB", " ", 3) : "no ripple limit";
        case Type::CableLoss:
            return knownLength > 0 ? Unit::ToString(knownLength, "m", "m k", 4) : "length unknown";
        default:
            break;
        }
        break;
...
QString Marker::tooltipSettings()
{
    ...
    case Trace::DataType::Time:
        ...
        case Type::CableImpedance:
            return "Impedance tolerance of the flat section search (entering a value restarts the automatic search)";
        default:
            break;
        }
        break;
    ...
    case Trace::DataType::Frequency:
        ...
        case Type::AntennaBand:
            return "VSWR threshold defining the band edges";
        case Type::Statistics:
            return "Ripple limit in dB peak-to-peak (0 = no check); the band is the marker restriction range";
        case Type::CableLoss:
            return "Length of the cable under test in m (0 = unknown, no dB/m); the band is the marker restriction range";
        default:
            break;
        }
        break;
...
void Marker::updateSymbol()
{
    if(isDisplayedMarker() && parentTrace) {
        ...
        case MarkerSymbolStyle::FilledNumberInside: {
            ...
            QPointF points[] = {QPointF(0.5,0.5),QPointF(width-0.5,0.5),QPointF(width/2,height-0.5)};
            auto traceColor = parentTrace->color();
            // outlined in the plot foreground so the symbol stands out on its own trace
            p.setPen(QPen(Theme::graphTextColor(), 1));
        ...
        case MarkerSymbolStyle::FilledNumberAbove: {
            ...
            QPainter p(&symbol);
            p.setRenderHint(QPainter::Antialiasing);
            QPointF points[] = {QPointF(0.5,height/2),QPointF(width-0.5,height/2),QPointF(width/2,height-0.5)};
            auto traceColor = parentTrace->color();
            p.setPen(QPen(Theme::graphTextColor(), 1));
            p.setBrush(traceColor);
            p.drawConvexPolygon(points, 3);
            p.setPen(Theme::readableOn(traceColor, Preferences::getInstance().Graphs.Color.background));
...
std::set<Marker::Type> Marker::getSupportedTypes()
{
    ...
    if(parentTrace) {
        ...
        case Trace::DataType::Time:
            ...
            if(parentTrace->isReflection()) {
                supported.insert(Type::CableImpedance);
            }
            break;
        case Trace::DataType::Frequency:
            ...
            supported.insert(Type::Statistics);
            if(!parentTrace->isReflection()) {
                supported.insert(Type::Lowpass);
                supported.insert(Type::Highpass);
                supported.insert(Type::Bandpass);
            } else {
                supported.insert(Type::AntennaBand);
                supported.insert(Type::CableLoss);
            }
...
void Marker::setType(Marker::Type t)
{
    ...
    case Type::AntennaBand:
        if(parentTrace->isReflection()) {
            required_helpers = {{"l", "lower band edge", Type::Manual}, {"h", "upper band edge", Type::Manual}};
        }
        break;
    case Type::CableImpedance:
        required_helpers = {{"s", "section start", Type::Manual}, {"e", "section end", Type::Manual}};
        break;
    case Type::Statistics:
    case Type::CableLoss:
        required_helpers = {{"l", "band start", Type::Manual}, {"h", "band end", Type::Manual}};
        break;
    default:
        break;
    }
    ...
    if(type == Type::CableImpedance) {
        sectionManual = false;
        for(auto h : helperMarkers) {
            h->formatTable = Format::Impedance;
            h->formatGraph.clear();
            h->formatGraph.insert(Format::Impedance);
            // the user may drag the section edges, keep them afterwards
            connect(h, &Marker::positionChanged, this, [=](){
                if(!sectionUpdating) {
                    sectionManual = true;
                    update();
                }
        ...
        if(Preferences::getInstance().Marker.defaultBehavior.showDataOnGraphs) {
            formatGraph.insert(Format::CableImpedance);
            formatGraph.insert(Format::CableLength);
            formatGraph.insert(Format::CableDelay);
        }
    }
    if(type == Type::Statistics) {
        for(auto h : helperMarkers) {
            h->formatTable = Format::dB;
            h->formatGraph.clear();
        }
        if(Preferences::getInstance().Marker.defaultBehavior.showDataOnGraphs) {
            if(parentTrace && parentTrace->isReflection()) {
                formatGraph.insert(Format::StatVSWR);
            }
            formatGraph.insert(Format::StatMagnitude);
            formatGraph.insert(Format::Ripple);
        }
    }
    if(type == Type::CableLoss) {
        for(auto h : helperMarkers) {
            h->formatTable = Format::dB;
            h->formatGraph.clear();
        }
        if(Preferences::getInstance().Marker.defaultBehavior.showDataOnGraphs) {
            formatGraph.insert(Format::CableLoss);
            formatGraph.insert(Format::CableLossBand);
        }
    }
    if(type == Type::AntennaBand) {
        // band edges only need to show the VSWR, the main marker shows the band data
        for(auto h : helperMarkers) {
            h->formatTable = Format::VSWR;
            h->formatGraph.clear();
            h->formatGraph.insert(Format::VSWR);
        }
        if(Preferences::getInstance().Marker.defaultBehavior.showDataOnGraphs) {
            formatGraph.insert(Format::Bandwidth);
            formatGraph.insert(Format::BandEdges);
        }
    }
    constrainFormat();
...
void Marker::setRestriction(bool enabled, double min, double max)
{
    if(max < min) {
        swap(min, max);
    }
    restrictPosition = enabled;
    minPosition = min;
    maxPosition = max;
    constrainPosition();
    update();
...
void Marker::setZTolerance(double tolerance)
{
    if(tolerance < 0.1) {
        tolerance = 0.1;
    }
    zTolerance = tolerance;
    update();
}

void Marker::setKnownLength(double length)
{
    knownLength = length > 0 ? length : 0.0;
    update();
}

void Marker::setNominalVF(double vf)
{
    nominalVF = vf > 0 ? vf : 0.0;
    update();
}

void Marker::setCableType(const QString &name)
{
    cableTypeName = CableLibrary::find(name) ? name : QString();
    update();
}

void Marker::resetSection()
{
    sectionManual = false;
    update();
}

void Marker::setRippleLimit(double dB)
{
    rippleLimit = dB > 0 ? dB : 0.0;
    update();
}

void Marker::setVSWRThreshold(double threshold)
{
    if(threshold < 1.01) {
        threshold = 1.01;
    }
...
nlohmann::json Marker::toJSON()
{
    ...
    case Type::PhaseNoise:
        ...
        break;
    case Type::AntennaBand:
        j["vswr_threshold"] = vswrThreshold;
        break;
    case Type::Statistics:
        j["ripple_limit"] = rippleLimit;
        break;
    case Type::CableLoss:
        j["known_length"] = knownLength;
        j["cable_type"] = cableTypeName.toStdString();
        break;
    case Type::CableImpedance:
        j["z_tolerance"] = zTolerance;
        j["known_length"] = knownLength;
        j["nominal_vf"] = nominalVF;
        j["section_manual"] = sectionManual;
        if(sectionManual && helperMarkers.size() >= 2) {
            j["section_start"] = helperMarkers[0]->position;
            j["section_end"] = helperMarkers[1]->position;
        }
        break;
    default:
...
void Marker::fromJSON(nlohmann::json j)
{
    ...
    case Type::PhaseNoise:
        ...
        break;
    case Type::AntennaBand:
        vswrThreshold = j.value("vswr_threshold", 2.0);
        break;
    case Type::Statistics:
        rippleLimit = j.value("ripple_limit", 0.0);
        break;
    case Type::CableLoss:
        knownLength = j.value("known_length", 0.0);
        cableTypeName = QString::fromStdString(j.value("cable_type", ""));
        break;
    case Type::CableImpedance:
        zTolerance = j.value("z_tolerance", 5.0);
        knownLength = j.value("known_length", 0.0);
        nominalVF = j.value("nominal_vf", 0.0);
        sectionManual = j.value("section_manual", false);
        if(sectionManual && helperMarkers.size() >= 2) {
            sectionUpdating = true;
            helperMarkers[0]->setPosition(j.value("section_start", 0.0));
            helperMarkers[1]->setPosition(j.value("section_end", 0.0));
            sectionUpdating = false;
        }
        break;
    default:
...
SIUnitEdit *Marker::getSettingsEditor()
{
    ...
    case Trace::DataType::Time:
        ...
        case Type::Delta:
            ...
            break;
        case Type::CableImpedance:
            ret = new SIUnitEdit("Ω", " ", 3);
            ret->setValue(zTolerance);
            break;
        default:
            return nullptr;
        }
    ...
    case Trace::DataType::Frequency:
        ...
        case Type::NegativePeakTable:
            ...
            break;
        case Type::AntennaBand:
            ret = new SIUnitEdit("", " ", 3);
            ret->setValue(vswrThreshold);
            break;
        case Type::Statistics:
            ret = new SIUnitEdit("dB", " ", 3);
            ret->setValue(rippleLimit);
            break;
        case Type::CableLoss:
            ret = new SIUnitEdit("m", "m k", 4);
            ret->setValue(knownLength);
            break;
        default:
            return nullptr;
        }
...
QWidget *Marker::getRestrictEditor()
{
    ...
    min->setPrecision(5);
    min->setMaxDecimals(3);
    Theme::fitWidth(min, "100.000 MHz");
    ...
    max->setPrecision(5);
    max->setMaxDecimals(3);
    Theme::fitWidth(max, "100.000 MHz");
...
void Marker::adjustSettings(double value)
{
    ...
    case Trace::DataType::Frequency:
        ...
        case Type::PhaseNoise:
            ...
            break;
        case Type::AntennaBand:
            // VSWR threshold must be above 1 (a perfect match has no band edges)
            if(value < 1.01) {
                value = 1.01;
            }
            vswrThreshold = value;
            break;
        case Type::Statistics:
            rippleLimit = value > 0 ? value : 0.0;
            break;
        case Type::CableLoss:
            knownLength = value > 0 ? value : 0.0;
            break;
        default:
            break;
        }
...
void Marker::update()
{
    ...
    case Type::Flatness: {
        ...
        break;
    case Type::AntennaBand: {
        if(!parentTrace->isReflection() || helperMarkers.size() < 2) {
            break;
        }
        // resonance: lowest reflection within the (optionally restricted) range
        auto resFreq = parentTrace->findExtremum(false, xmin, xmax);
        setPosition(resFreq);
        // VSWR threshold expressed as reflection magnitude
        const double gammaMax = (vswrThreshold - 1.0) / (vswrThreshold + 1.0);
        const int size = parentTrace->size();
        int index = parentTrace->index(resFreq);
        if(index < 0) {
            index = 0;
        } else if(index >= size) {
            index = size - 1;
        }
        auto res = parentTrace->sample(index);
        bandFound = abs(res.y) <= gammaMax;
        double low = res.x;
        double high = res.x;
        bandLowClipped = false;
        bandHighClipped = false;
        if(bandFound) {
            ...
            auto crossing = [gammaMax](const TraceMath::Data &outside, const TraceMath::Data &inside) -> double {
                auto aOut = abs(outside.y);
                auto aIn = abs(inside.y);
                if(aOut <= aIn) {
                    return inside.x;
                }
                return outside.x + (aOut - gammaMax) / (aOut - aIn) * (inside.x - outside.x);
            };
            // walk towards lower frequencies while the VSWR stays below the threshold
            int i = index;
            bool clipped = true;
            while(i > 0) {
                auto prev = parentTrace->sample(i - 1);
                if(prev.x < xmin) {
                    break;
                }
                if(abs(prev.y) > gammaMax) {
                    clipped = false;
                    low = crossing(prev, parentTrace->sample(i));
                    break;
                }
                i--;
            }
            if(clipped) {
                low = parentTrace->sample(i).x;
            }
            bandLowClipped = clipped;
            // same towards higher frequencies
            i = index;
            clipped = true;
            while(i < size - 1) {
                auto next = parentTrace->sample(i + 1);
                if(next.x > xmax) {
                    break;
                }
                if(abs(next.y) > gammaMax) {
                    clipped = false;
                    high = crossing(next, parentTrace->sample(i));
                    break;
                }
                i++;
            }
            if(clipped) {
                high = parentTrace->sample(i).x;
            }
            bandHighClipped = clipped;
            // threshold line between the band edges (drawn by XY plots)
            lines.resize(1);
            lines[0].p1.x = low;
            lines[0].p1.y = complex<double>(gammaMax, 0.0);
            lines[0].p2.x = high;
            lines[0].p2.y = complex<double>(gammaMax, 0.0);
        }
        helperMarkers[0]->setPosition(low);
        helperMarkers[1]->setPosition(high);
    }
        break;
    case Type::Statistics: {
        stats = BandStats();
        if(helperMarkers.size() < 2 || getDomain() != Trace::DataType::Frequency) {
            break;
        }
        // samples inside the (optionally restricted) band
        const int size = parentTrace->size();
        vector<double> xs, mag, vswr;
        const bool reflection = parentTrace->isReflection();
        for(int i=0;i<size;i++) {
            auto s = parentTrace->sample(i);
            if(s.x < xmin || s.x > xmax || !isfinite(abs(s.y))) {
                continue;
            }
            xs.push_back(s.x);
            mag.push_back(Util::SparamTodB(s.y));
            if(reflection) {
                auto g = abs(s.y);
                vswr.push_back(g < 1.0 ? (1.0 + g) / (1.0 - g) : numeric_limits<double>::infinity());
            }
        }
        if(xs.size() < 2) {
            break;
        ...
        for(size_t i=0;i<mag.size();i++) {
            if(mag[i] < mag[iMin]) iMin = i;
            if(mag[i] > mag[iMax]) iMax = i;
            sum += mag[i];
        }
        stats.fLow = xs.front();
        stats.fHigh = xs.back();
        stats.magMin = mag[iMin];
        stats.magMax = mag[iMax];
        stats.magMean = sum / mag.size();
        if(reflection) {
            double vsum = 0;
            for(auto v : vswr) vsum += v;
            stats.vswrMin = vswr[iMin];
            stats.vswrMax = vswr[iMax];
            stats.vswrMean = vsum / vswr.size();
        }
        // ripple: peak-to-peak of |S| after removing the slow trend with a centered moving
        // average (window ~10 % of the band) so that the resonance dip itself does not count
        {
            const int n = (int) mag.size();
            int win = max(5, n / 10);
            int half = win / 2;
            double rMin = numeric_limits<double>::max(), rMax = numeric_limits<double>::lowest();
            for(int i=0;i<n;i++) {
                int a = max(0, i - half), b = min(n - 1, i + half);
                double acc = 0;
                for(int k=a;k<=b;k++) acc += mag[k];
                double r = mag[i] - acc / (b - a + 1);
                rMin = min(rMin, r);
                rMax = max(rMax, r);
            }
            stats.ripplePP = rMax - rMin;
        }
        stats.valid = true;
        // main marker at the worst point: highest reflection / lowest transmission
        setPosition(reflection ? xs[iMax] : xs[iMin]);
        helperMarkers[0]->setPosition(stats.fLow);
        helperMarkers[1]->setPosition(stats.fHigh);
    }
        break;
    case Type::CableLoss: {
        stats = BandStats();
        ...
        // one port cable loss: the far end of the cable is open (or shorted), so everything that
        // comes back has passed the cable twice -> one way loss = -|S11| dB / 2
        const int size = parentTrace->size();
        vector<double> xs, raw;
        for(int i=0;i<size;i++) {
            auto s = parentTrace->sample(i);
            auto mag = abs(s.y);
            if(s.x < xmin || s.x > xmax || !isfinite(mag) || mag <= 0) {
                continue;
            }
            xs.push_back(s.x);
            raw.push_back(-Util::SparamTodB(s.y) / 2);
        }
        const int n = (int) raw.size();
        if(n < 2) {
            break;
        }
        // the mismatch of the open/short end makes |S11| ripple around the true loss; a centered
        // moving average over ~5 % of the band removes it, the residual is reported as ripple
        int win = max(5, n / 20);
        int half = win / 2;
        ...
        for(int i=0;i<n;i++) {
            int a = max(0, i - half), b = min(n - 1, i + half);
            double acc = 0;
            for(int k=a;k<=b;k++) acc += raw[k];
            smooth[i] = acc / (b - a + 1);
            double d = raw[i] - smooth[i];
            rMin = min(rMin, d);
            rMax = max(rMax, d);
        }
        int iMin = 0, iMax = 0;
        double sum = 0;
        for(int i=0;i<n;i++) {
            if(smooth[i] < smooth[iMin]) iMin = i;
            if(smooth[i] > smooth[iMax]) iMax = i;
            sum += smooth[i];
            if(raw[i] < -0.25) {
                // |S11| well above 0 dB: more power comes back than went in, the data can not be calibrated
                stats.lossSuspicious = true;
            }
        }
        stats.fLow = xs.front();
        stats.fHigh = xs.back();
        stats.lossMin = smooth[iMin];
        stats.lossMax = smooth[iMax];
        stats.lossMean = sum / n;
        stats.ripplePP = rMax - rMin;
        stats.valid = true;
        // main marker at the worst case: the highest loss of the band
        setPosition(xs[iMax]);
        helperMarkers[0]->setPosition(stats.fLow);
        helperMarkers[1]->setPosition(stats.fHigh);
        // show the smoothed reflection the loss was derived from as a trend line
        const int segments = min(n - 1, 100);
        lines.resize(segments);
        for(int i=0;i<segments;i++) {
            int a = (int) ((double) i * (n - 1) / segments);
            int b = (int) ((double) (i + 1) * (n - 1) / segments);
            lines[i].p1.x = xs[a];
            lines[i].p1.y = complex<double>(pow(10.0, -smooth[a] / 10.0), 0.0);
            lines[i].p2.x = xs[b];
            lines[i].p2.y = complex<double>(pow(10.0, -smooth[b] / 10.0), 0.0);
        }
    }
        break;
    case Type::CableImpedance: {
        if(helperMarkers.size() < 2 || getDomain() != Trace::DataType::Time) {
            break;
        }
        const int size = parentTrace->size();
        const double z0 = parentTrace->getReferenceImpedance();
        // impedance from the step response, NaN where it is not available or at open/short
        auto zAt = [&](int i) -> double {
            double step = parentTrace->sample(i, true).y.real();
            if(isnan(step) || abs(step) >= 1.0) {
                return numeric_limits<double>::quiet_NaN();
            }
            return Util::SparamToImpedance(step, z0).real();
        };
        // only causal part of the response, optionally restricted by the user
        double searchMin = xmin > 0.0 ? xmin : 0.0;
        cableValid = false;
        cableClipped = false;
        cableEndKind = 0;
        // a run must be at least ~0.5 ns of round trip time (5 cm of cable) to count as a plateau
        double dt = size > 1 ? parentTrace->sample(1).x - parentTrace->sample(0).x : 0.0;
        int minCount = 3;
        if(dt > 0) {
            minCount = max(3, (int) ceil(0.5e-9 / dt));
        }
        int sectionStartIdx = -1, sectionEndIdx = -1;
        if(!sectionManual) {
            // split the causal response into runs whose impedance stays within the tolerance
            // of the running mean; the cable is the FIRST plateau after the calibration plane,
            // everything behind its far end can not be the cable
            struct Run { int start, end, count; bool clipped; };
            vector<Run> runs;
            int runStart = -1, runEnd = -1, runCount = 0;
            double runSum = 0.0;
            int lastIndex = -1;
            auto closeRun = [&](bool clipped) {
                if(runCount > 0) {
                    runs.push_back({runStart, runEnd, runCount, clipped});
                }
                runStart = -1;
                runEnd = -1;
                runCount = 0;
            ...
            for(int i = 0; i < size; i++) {
                double x = parentTrace->sample(i).x;
                if(x < searchMin) {
                    continue;
                }
                if(x > xmax) {
                ...
                double mean = runSum / runCount;
                if(fabs(z - mean) > zTolerance) {
                    closeRun(false);
                    runStart = i;
                    runEnd = i;
                    runCount = 1;
            ...
            closeRun(runEnd >= 0 && runEnd == lastIndex);
            // first sufficiently long plateau, otherwise the longest run available
            ...
            if(!best) {
                for(auto &r : runs) {
                    if(r.count >= 3 && (!best || r.count > best->count)) {
                        best = &r;
                    }
                }
            }
            ...
            // keep the untrimmed edges for the delay, average over the inner part only
            sectionStartIdx = best->start;
            sectionEndIdx = best->end;
            cableStartTime = parentTrace->sample(best->start).x;
            cableEndTime = parentTrace->sample(best->end).x;
            cableClipped = best->clipped;
            int trim = (best->end - best->start) / 10;
            sectionUpdating = true;
            helperMarkers[0]->setPosition(parentTrace->sample(best->start + trim).x);
            helperMarkers[1]->setPosition(parentTrace->sample(best->end - trim).x);
            sectionUpdating = false;
        } else {
            cableStartTime = min(helperMarkers[0]->position, helperMarkers[1]->position);
            cableEndTime = max(helperMarkers[0]->position, helperMarkers[1]->position);
            sectionStartIdx = parentTrace->index(cableStartTime);
            sectionEndIdx = parentTrace->index(cableEndTime);
        }
        // is the far end open or shorted? look for a large step (|rho| > 0.5) right behind the section
        if(!cableClipped && sectionEndIdx >= 0) {
            int lookahead = max(10, minCount * 4);
            for(int i = sectionEndIdx + 1; i < size && i <= sectionEndIdx + lookahead; i++) {
                double step = parentTrace->sample(i, true).y.real();
                if(!isnan(step) && fabs(step) > 0.5) {
                    cableEndKind = step > 0 ? 1 : -1;
                    break;
                }
            }
        }
        // statistics over the section between the helper markers
        double from = min(helperMarkers[0]->position, helperMarkers[1]->position);
        double to = max(helperMarkers[0]->position, helperMarkers[1]->position);
        int i0 = parentTrace->index(from);
        int i1 = parentTrace->index(to);
        if(i0 < 0) {
            i0 = 0;
        }
        if(i1 >= size) {
        ...
        if(count > 0) {
            cableMeanZ = sum / count;
            cableMinZ = zmin;
            cableMaxZ = zmax;
            cableValid = true;
        }
        setPosition((from + to) / 2);
    }
        break;
    case Type::Last:
        break;
...
{
    if(parent) {
        if(parent->type == Type::Flatness || parent->type == Type::CableImpedance) {
            // flatness and cable impedance are the exception, they have movable helper markers
            return true;
        }
