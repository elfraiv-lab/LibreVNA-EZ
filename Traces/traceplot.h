...
public:
    ...
    // Width (px) this plot would like its tile to have for the given tile height,
    // or -1 if it simply fills whatever space it gets. Used by TileWidget to keep
    // circular charts square instead of wasting the space next to them.
    virtual int preferredTileWidth(int tileHeight) { Q_UNUSED(tileHeight) return -1; }
...
signals:
    ...
    // emitted when the result of preferredTileWidth() may have changed
    void tileFitChanged();
...
protected:
    ...
    int markerDataWidth;    // width of the marker data column drawn on the right (0 if none)

    bool limitPassing;

private:
    QString title;
    bool showMarkerData;    // draw marker values on this graph
};
