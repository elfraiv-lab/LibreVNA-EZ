...
int TracePolar::preferredTileWidth(int tileHeight)
{
    if(!fitTile) {
        return -1;
    }
    // the chart is a circle whose diameter is the drawing area height;
    // marker data (if shown) needs its own column to the right of it
    int w = tileHeight - (int) marginTop - (int) marginBottom + markerDataWidth;
    return w > 0 ? w : -1;
}
...
void TracePolar::updateContextMenu()
{
    ...
    auto fit = new QAction("Fit tile to chart", contextmenu);
    fit->setCheckable(true);
    fit->setChecked(fitTile);
    contextmenu->addAction(fit);
    connect(fit, &QAction::toggled, this, [=](bool on) {
        fitTile = on;
        emit tileFitChanged();
    });
