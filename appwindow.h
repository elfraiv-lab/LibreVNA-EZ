...
public:
    ...
    // Compact arrangement for small touch screens: docks tabbed on the right, log hidden, larger icons
    void ApplyTabletLayout();
    void ApplyDesktopLayout();
    // enforce global UI preferences after a mode restored its window state
    void AfterLayoutRestore();
    // release frozen dock sizes so that QMainWindow::restoreState / layout resets can apply
    void PrepareLayoutRestore();
...
private:
    void SetupViewMenu();
    void SetLayoutMode(bool tablet);
    // dock panels: freeze features/sizes (UI.panelsLocked) and hide/show (UI.panelsHidden)
    void ApplyPanelLock();
    void ApplyPanelsHidden();
    QAction *graphColorsAction = nullptr; // View > "Graph colors follow theme"
