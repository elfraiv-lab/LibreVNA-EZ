...
QString Unit::ToString(double value, QString unit, QString prefixes, int precision, int maxDecimals)
{
    // change label text
    QString sValue;
    if(isnan(value) || isinf(value)) {
        ...
        if(preDotDigits >= 0) {
            if(precision - preDotDigits < 0) {
                ...
                decimals = precision - preDotDigits;
            }
        } else {
            decimals = precision - 2;
        }
        if(maxDecimals >= 0 && decimals > maxDecimals) {
