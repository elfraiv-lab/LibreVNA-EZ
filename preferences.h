...
public:
    ...
    struct {
        bool darkTheme;     // legacy flag, kept in sync with theme != Light
        int theme;          // Theme::Kind: 0 light, 1 dark, 2 acid green, 3 phosphor scope; -1 = derive from darkTheme
        bool tabletMode;    // enlarged controls, compact dock layout, numpad input
        bool vnaOnly;       // hide Signal Generator / Spectrum Analyzer modes
        bool showReferenceToolbar; // Ref in / Ref out toolbar
        bool compactToolbar;       // hide rarely used VNA toolbar items (sweep type, log, dwell, averaging)
        bool graphColorsFollowTheme; // Graphs.Color taken from the theme (cleared when edited in the preferences dialog)
        double minFrequency;       // lower limit for sweeps and graph X axis (Hz)
        double maxFrequency;       // upper limit for graph X axis (Hz)
        // last values of the "Antenna band" quick setup dialog
        double antennaBandStart;
        double antennaBandStop;
        double antennaBandVSWR;
        double antennaBandMargin;  // percent of the bandwidth
        QString antennaReportName; // last antenna name used in the PDF report dialog
        // last values of the "Cable impedance (TDR)" quick setup dialog
        bool cableKnownLength;
        double cableLength;        // m
        double cableVelocityFactor;
        double cableZTolerance;    // Ω
        QString cableType;         // cable library entry selected in the TDR dialog
        // last values of the "Cable loss (open/short)" quick setup dialog
        QString cableLossType;     // cable library entry or "Custom"
        double cableLossLength;    // m, 0 = unknown
        double cableLossStart;     // Hz
        double cableLossStop;      // Hz
        // cable between calibration plane and antenna (port extension), "Antenna band" dialog
        QString antennaCableType;  // "None", library entry or "Custom"
        double antennaCableLength; // m
        double antennaCableVF;     // custom cable
        double antennaCableLoss;   // custom cable, dB/m at 1 GHz
        // dock panels (marker, traces, log): frozen arrangement / hidden by the toolbar buttons
        bool panelsLocked;
        bool panelsHidden;
        QString panelsHiddenList;  // titles of the docks hidden by the button, ';' separated
    } UI;
    struct {
        struct {
            unsigned int ports;
...
private:
    ...
    const std::vector<Savable::SettingDescription> descr = {{
        ...
        {&UI.darkTheme, "UI.darkTheme", false},
        {&UI.theme, "UI.theme", -1},
        {&UI.tabletMode, "UI.tabletMode", false},
        {&UI.vnaOnly, "UI.vnaOnly", true},
        {&UI.showReferenceToolbar, "UI.showReferenceToolbar", false},
        {&UI.compactToolbar, "UI.compactToolbar", true},
        {&UI.graphColorsFollowTheme, "UI.graphColorsFollowTheme", true},
        {&UI.minFrequency, "UI.minFrequency", 100000.0},
        {&UI.maxFrequency, "UI.maxFrequency", 6000000000.0},
        {&UI.antennaBandStart, "UI.antennaBandStart", 2400000000.0},
        {&UI.antennaBandStop, "UI.antennaBandStop", 2500000000.0},
        {&UI.antennaBandVSWR, "UI.antennaBandVSWR", 2.0},
        {&UI.antennaBandMargin, "UI.antennaBandMargin", 20.0},
        {&UI.antennaReportName, "UI.antennaReportName", ""},
        {&UI.cableKnownLength, "UI.cableKnownLength", false},
        {&UI.cableLength, "UI.cableLength", 1.0},
        {&UI.cableVelocityFactor, "UI.cableVelocityFactor", 0.66},
        {&UI.cableZTolerance, "UI.cableZTolerance", 5.0},
        {&UI.cableType, "UI.cableType", "RG-58 A/U (FPE)"},
        {&UI.cableLossType, "UI.cableLossType", "RG-58 A/U (FPE)"},
        {&UI.cableLossLength, "UI.cableLossLength", 10.0},
        {&UI.cableLossStart, "UI.cableLossStart", 100000.0},
        {&UI.cableLossStop, "UI.cableLossStop", 3000000000.0},
        {&UI.antennaCableType, "UI.antennaCableType", "None"},
        {&UI.antennaCableLength, "UI.antennaCableLength", 1.0},
        {&UI.antennaCableVF, "UI.antennaCableVF", 0.66},
        {&UI.antennaCableLoss, "UI.antennaCableLoss", 0.5},
        {&UI.panelsLocked, "UI.panelsLocked", false},
        {&UI.panelsHidden, "UI.panelsHidden", false},
        {&UI.panelsHiddenList, "UI.panelsHiddenList", ""},
        ...
        {&UISettings.Paths.report, "UISettings.Paths.report", ""},
