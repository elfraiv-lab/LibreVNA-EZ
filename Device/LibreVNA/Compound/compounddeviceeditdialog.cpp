...
void DeviceFrame::update()
{
    ...
    if(dev->sync == LibreVNADriver::Synchronization::GUI) {
        setStyleSheet("image: url(:/icons/compound_V1_USB.svg);");
    } else if(dev->sync == LibreVNADriver::Synchronization::Reserved) {
        if(position == 0) {
            setStyleSheet("image: url(:/icons/compound_V1_Ref_Left.svg);");
        } else if(position == dev->deviceSerials.size() - 1) {
            setStyleSheet("image: url(:/icons/compound_V1_Ref_Right.svg);");
        } else {
            setStyleSheet("image: url(:/icons/compound_V1_Ref_Middle.svg);");
        }
    }
}
