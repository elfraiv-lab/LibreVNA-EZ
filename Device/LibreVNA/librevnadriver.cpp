...
void LibreVNADriver::handleReceivedPacket(const Protocol::PacketInfo &packet)
{
    ...
    case Protocol::PacketType::DeviceInfo: {
        ...
        // firmware allows sweeps below the specified range; the GUI enforces the
        // configured lower limit (UI.minFrequency, 100 kHz by default)
        info.Limits.VNA.minFreq = std::max((double) packet.info.limits_minFreq, Preferences::getInstance().UI.minFrequency);
